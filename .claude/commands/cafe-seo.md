---
description: "카페SEO 원고 파이프라인 실행. 키워드 입력 → 제목 → 본문 → 댓글 → 검수 → 저장까지 자동."
---

카페SEO 원고를 자동으로 생성한다. 사용자에게 중간 승인을 묻지 않는다. 최종 결과만 보고한다.
...
```bash
python3 -m src.pipeline_v2.cafe_seo \
  --keyword "{키워드}" \
  --product-name "{제품명}" \
...
중단된 작업을 이어서 실행할 때:
```bash
python3 -m src.pipeline_v2.cafe_seo --resume {job_id}
```
