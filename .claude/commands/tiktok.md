---
description: "틱톡 스크립트 파이프라인 실행"
---

틱톡 스크립트를 자동으로 생성한다. 사용자에게 중간 승인을 묻지 않는다. 최종 결과만 보고한다.
...
- 키워드, 제품명, 브랜드키워드, USP, 타겟, 성분, 소구점, 구매원씽
...
```bash
python3 -m src.pipeline_v2.tiktok \
  --keyword "{키워드}" \
  --product-name "{제품명}" \
...
중단된 작업을 이어서 실행할 때:
```bash
python3 -m src.pipeline_v2.tiktok --resume {job_id}
```
