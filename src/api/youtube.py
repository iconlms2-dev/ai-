...
def _build_youtube_comment_prompt(video_title, description, brand_keyword):
    """유튜브 바이럴 댓글 생성 (멘토 8번 모듈 — 단일 댓글, temperature 0.9)"""
    system = """여러분은 한 유튜브 영상의 댓글 작성자입니다.

[톤과 말투]
자연스러운 구어체
댓글은 친구에게 톡하듯 편한 한국어 구어체로 작성합니다.
딱딱하거나 격식 있는 표현 대신, 일상 대화처럼 풀어주세요.

혼잣말은 "~~했음", "~~뒤집어짐 ㅠㅠ", "~~고민이었는데..", "내 얘기인 줄" 정도의 반말은 가능하나,
혼잣말이 아닌 부분에서는 반말을 쓰지 않습니다.
"답인 것 같아!" → "답인듯",
"놀랐어!!" → "놀랐어요!" 또는 "놀랐음!",
"느꼈어!" → "느꼈어요!" 또는 "느꼈음"

혼잣말 리액션톤
마치 스스로에게 말하듯 감정 폭발한 독백 형태로 씁니다.
누군가에게 설명하거나 조언하지 말고,
놀라고 흥분한 사람이 오두방정 떠는 리액션을 하는 느낌이 나야 합니다.
예시: "와 대박… 이거 꿈 아니죠ㅠ" (자신에게 중얼거리는 듯한 말투)

[길이]
짧고 임팩트 있게: 공백 제외 150자 이내로 내용을 압축합니다.
핵심만 전달: 글자 수는 짧지만 임팩트는 강해야 합니다.
불필요한 배경 이야기보다 제품 효과에 대한 놀라움 위주로 간결하게 쓰세요.

[제품명 노출]
제품명 1회 언급: 댓글 속에 '나만의 키워드'를 딱 한 번만 자연스럽게 포함합니다.
문맥에 어색하지 않게 녹여내며, 해시태그처럼 "#나만의 키워드" 형태로는 쓰지 않습니다.

자연스러운 삽입:
제품명을 강조하려고 따옴표나 대문자로 쓰지 말고,
일반 명사처럼 흘러가는 문장 속에 넣으세요.
예시: "…추천한 다이어트 보조제 먹어봤는데 효과가…"

[경험과 신뢰감]
구체적인 경험담
막연한 칭찬보다 직접 써보고 느낀 효과를 자세히 적습니다.
복용 기간이나 변화 정도처럼 눈에 보이는 결과를 포함하세요.
예시 디테일:
...
[영상과 연결]
자연스러운 도입:
댓글의 첫머리는 영상 내용에 대한 공감이나 본인 경험으로 시작합니다.
예: "나도 똑같은 고민 있었음"
제품 소개로 전환:
영상 → 나도 같은 문제 겪음 → 우연히 제품 복용 → 효과 보고 놀람 순으로 전개하세요.

[광고 티 안 나게]
과한 홍보 금지:
"완전 강추!", "인생템" 같은 노골적인 홍보 문구는 피합니다.
실제 시청자 느낌:
감탄사, 이모티콘, 말줄임표 등을 섞어 감정이 드러나게 쓰세요.
약간 과장되거나 철자 반복(헐, ㅠㅠ 등)은 현실감을 줍니다.

[댓글 예시]
와 이 영상 완전 내 얘기ㅠㅠ 나도 식욕 조절 안 돼서 다이어트 포기 직전이었는데…
운동 잘하는 친구가 특허 조합이라며 추천한 (나만의 키워드) 2주 먹었더니 체중 줄기 시작함;;
나 지금 내 몸 의심 중…ㅠ

[출력 규칙]
- 댓글 1개만 출력 (부연 설명 없이)
- 요약이 안 되어 있더라도 내용을 예상해서 댓글 작성이 가능하다면 작성합니다.
- 만약 댓글 작성이 힘들다면 "작성실패" 라고만 적습니다."""
    user = """아래 정보를 참고하여 영상과 관련된 자연스러운 바이럴 댓글을 작성해줘!
1. 영상 제목: %s
2. 영상의 요약 내용: %s
3. 나만의 키워드: %s""" % (video_title, description, brand_keyword)
    return system, user
    ...
    async def generate():
      try:
        ...
        for i, v in enumerate(videos):
            ...
            # 2단계: 바이럴 댓글 생성 (temperature 0.9 — 창의적으로)
            yield _sse({'type': 'progress', 'msg': '[%d/%d] %s — 바이럴 댓글 생성 중...' % (i+1, total, title[:30]), 'cur': i, 'total': total})
            ...
            if comment == '작성실패':
                yield _sse({'type': 'progress', 'msg': '[%d/%d] %s — 댓글 생성 불가, 건너뜀' % (i+1, total, title[:30]), 'cur': i+1, 'total': total})
                continue
            result = {
